...
if [[ ! -f "${MONITORING_ENV}" ]]; then
  printf 'Missing monitoring env: %s\n' "${MONITORING_ENV}" >&2
  exit 1
fi
...
for script_name in backup_postgres.sh create_secrets.sh preflight.sh restore_postgres.sh; do
  install -m 0755 \
    "${RELEASE_DIR}/infra/production/aws-ec2/bin/${script_name}" \
...
COMPOSE_FILE="${CURRENT_LINK}/infra/production/aws-ec2/docker-compose.prod.yml"
...
for _ in $(seq 1 30); do
  if "${COMPOSE[@]}" exec -T api python -c 'import urllib.request; urllib.request.urlopen("http://127.0.0.1:8000/health/ready", timeout=5)' >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
